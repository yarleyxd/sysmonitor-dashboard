...
function criarIndicador(titulo, valor) {
    ...
    const nome = criarElemento("span", "nome-indicador", titulo);
    const porcentagem = criarElemento("span", "valor-indicador", valor);

    const barra = criarElemento("div", "barra");

    const preenchimento = criarElemento("div", "preenchimento");

    const numero = Number.parseFloat(valor);

    if (Number.isFinite(numero)) {
        preenchimento.style.width = `${Math.min(100, Math.max(0, numero))}%`;
    }
...
function criarComputador(desktop) {
    const computador = criarElemento("article", "computador");

    const cabecalho = criarElemento("div", "computador-cabecalho");

    const nome = criarElemento("h3", "", desktop.nome || "Nome desconhecido");

    const status = criarElemento(
        "span",
        "status " + (
            desktop.status === "Online" ? "online" : "offline"
        ),
    ...
    informacoes.appendChild(
        criarElemento("p", "", `IP: ${desktop.ip || "Não informado"}`)
    );
...
function atualizarResumo(desktops) {
    ...
    const mediaCPU = cpus.length
        ? cpus.reduce((soma, valor) => soma + valor, 0) / cpus.length
        : 0;

    const mediaRAM = rams.length
        ? rams.reduce((soma, valor) => soma + valor, 0) / rams.length
        : 0;
    ...
    try {
        ...
        if (desktops.length === 0) {
            ...
            desktops.forEach(desktop => {
                lista.appendChild(criarComputador(desktop));
            });
        }
        ...
        statusServidor.textContent = "Erro de conexão";
